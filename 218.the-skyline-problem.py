...
class Solution:
    def getSkyline(self, buildings: List[List[int]]) -> List[List[int]]:
        ...
        while hq:
            new_point = hq[0][0]
            while hq and new_point == hq[0][0]:
                start, status, index = heappop(hq)
                if status == 0:
                    remove.add(index)
                else:
                    heappush(tq, (-buildings[index][2], index))
            while tq and tq[0][1] in remove:
                heappop(tq)
            if tq:
                if curr != -tq[0][0]:
                    rst.append([new_point, -tq[0][0]])
                    curr = -tq[0][0]
            else:
                rst.append([new_point, 0])
                curr = 0
